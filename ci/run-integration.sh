...
#   VGI_CHEM_WORKER  worker LOCATION the .test files ATTACH (a stdio command
#                          such as `uv run chem_worker.py`, or an http:// URL)
# Optional:
#   STAGE                  scratch dir for the preprocessed test tree (default: mktemp)
set -euo pipefail

: "${HAYBARN_UNITTEST:?path to the haybarn-unittest binary}"
: "${VGI_CHEM_WORKER:?worker LOCATION (stdio command or http:// URL)}"
...
echo "Running suite (worker: $VGI_CHEM_WORKER) ..."
