...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-chem
...
# ATTACH (`${VGI_CHEM_WORKER}`):
#
#   subprocess : a bare stdio command (`uv run chem_worker.py`) — the
...
#                        `uv run --python 3.13 <repo>/chem_worker.py`.
# Optional:
...
WORKER_CMD="${WORKER_CMD:-uv run --python 3.13 $REPO/chem_worker.py}"
...
# ---------------------------------------------------------------------------
# Per-transport: resolve VGI_CHEM_WORKER (the LOCATION) and, for the
...
case "$TRANSPORT" in
  subprocess)
    export VGI_CHEM_WORKER="$WORKER_CMD"
    ;;
  ...
  http)
    ...
    PORT_FILE="$(mktemp -u "${TMPDIR:-/tmp}/chem-port.XXXXXX")"
    LOG_FILE="${TMPDIR:-/tmp}/chem-http-server.log"
    ...
    export VGI_CHEM_WORKER="http://127.0.0.1:$PORT"
    ;;
  ...
  unix)
    ...
    SOCK="${TMPDIR:-/tmp}/chem-$$.sock"
    rm -f "$SOCK"
    LOG_FILE="${TMPDIR:-/tmp}/chem-unix-server.log"
    ...
    export VGI_CHEM_WORKER="unix://$SOCK"
    ;;
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_CHEM_WORKER) ..."
